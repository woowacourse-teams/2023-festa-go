...
object MainBinding {
    fun setBtnEnterTicket(binding: ActivityMainBinding, ticket: TicketUiModel) {
        ...
        when (LocalDateTime.now().isAfter(ticket.entryTime)) {
            false -> {
                ...
                binding.btnEnterTicket.text = context.getString(R.string.main_ticket_enable_time)
    ...
    fun setTvTicketState(binding: ActivityMainBinding, ticket: TicketUiModel) {
        ...
        binding.tvTicketState.text = when (ticket.ticketState) {
            TicketStateUiModel.BEFORE_ENTRY -> context.getString(R.string.main_ticket_before_entry)
            TicketStateUiModel.AFTER_ENTRY -> context.getString(R.string.main_ticket_after_entry)
            TicketStateUiModel.AWAY -> context.getString(R.string.main_ticket_away)
            else -> ""
        }
    }
}
